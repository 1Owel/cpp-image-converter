...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    std::vector<JSAMPLE> scanline(row_stride);

    while (cinfo.next_scanline < cinfo.image_height) {
        const Color* row = image.GetLine(cinfo.next_scanline);

        for (int x = 0; x < image.GetWidth(); ++x) {
            scanline[x * 3 + 0] = static_cast<JSAMPLE>(row[x].r);
            scanline[x * 3 + 1] = static_cast<JSAMPLE>(row[x].g);
            scanline[x * 3 + 2] = static_cast<JSAMPLE>(row[x].b);
        }
